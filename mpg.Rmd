...
## Executive Summary
Looking at the `mtcars` data set, the relationship between the various variables and the miles per gallon (mpg) outcome is explored. Specifically, the following questions are answered:

1. Is an automatic or manual transmission better for MPG?
2. Quantify the MPG difference between automatic and manual transmissions?

The analysis of the data shows that the vehicle transmission type is one of the significant variables defining the miles per gallon of the vehicle.
...
Figure 1 (in the appendix) looks at the mpg of the various vehicles divided by transmission type. The mpg for automatic transmission vehicles have mean of `r round(mean(mtcars$mpg[mtcars$am == 0]), 2)` while manual transmission vehicles have mean of `r round(mean(mtcars$mpg[mtcars$am == 1]), 2)`.
...
```{r}
pval <- summary(lm(mpg~am, mtcars))$coefficients[2,4]
```
